 	...
 	withEnv(["HOME=${env.WORKSPACE}"]) {	
		...
		withCredentials([file(credentialsId: SERVER_KEY_CREDENTIALS_ID, variable: "server_key_file")]) {
			...
			stage('Authorize to Salesforce') {
				...
				rc = bat returnStatus: true, script: "\"${toolbelt}\" force:auth:jwt:grant --clientid ${SF_CONSUMER_KEY} --username ${SF_USERNAME} --jwtkeyfile \"${server_key_file}\" --setalias DevOrg --instanceurl ${SF_INSTANCE_URL}"
				if (rc != 0) {
